    ...
    if (err || !user) {
      ...
      req.login(user, function (err) {
        if (err) {
          ...
          res.json({token: user.token, roles: user.roles, context: user.context, displayName: user.displayName,profileImageURL: user.profileImageURL});
        }
      });
    }
  })(req, res, next);
  ...
  User.findOne({where:{token: token}})
    .then(function(user){
      ...
      user.updateAttributes({
        ...
        req.token = undefined;
        req.logout();
        res.jsonp({message: 'user disconnected successfully'});
      }).catch(function(err){
        res.status(400).send({message: 'ohh : something was wrong'});
      });
    ...
    .catch(function(err){
      req.token = undefined;
      res.status(400).send({message: 'session has already expired'});
    });
